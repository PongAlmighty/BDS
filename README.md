...
- **StrangerTV Corner Hits (predicted)**: Polls the kiosk's status endpoint and drops 3D hex nuts (not beans) *a few seconds before* its bouncing DVD nut lands in a corner, so the drop is the cue to look up and watch it happen rather than an announcement that you missed it. The kiosk's bounce is deterministic, so it tabulates its own hit times exactly and publishes a `corner_warn` ahead of each one -- this is a lookup, not a guess. Polling rather than push because the kiosk cannot reach this machine. Set `KIOSK_STATUS_URL` to enable, blank to disable.
  - Advance notice is `KIOSK_CORNER_WARN` seconds on the kiosk (default 15). BDS only sees it on its next poll, so the notice actually delivered is between the full lead and lead-minus-`KIOSK_POLL_INTERVAL` -- roughly **13-15 seconds** at the defaults. That value lives on the kiosk, not here, so changing the lead means redeploying StrangerTV.
  - The warning nut despawns after `CORNER_WARN_LIFETIME` seconds (default 10) instead of the usual 60, so it is gone before the hit lands rather than sitting in shot through it. The banner that comes with it ignores the floor and everything on it, drifting down and out of frame in about the same time -- cheer and redeem banners still land on the pile as before.
  ...
  - The warning drops **one nut at `CORNER_WARN_SCALE`x** (default 2.5) rather than a shower of small ones, so it reads as "look up now" at a glance. The drop point is clamped so an oversized nut cannot spawn overlapping the side walls; past about 15x it is taller than the frustum. Preview it with `/test/warn`.
...
| `/test/nut?count=120&scale=1&lifetime=60&text=CORNER%20HIT!` | Hex nuts, arbitrary count. `scale` multiplies nut size, `lifetime` is seconds on screen. `text=0` suppresses the banner. |
| `/test/warn` | Fires exactly what a real corner warning fires -- one nut at `CORNER_WARN_SCALE` for `CORNER_WARN_LIFETIME` seconds. |
